...
public class HttpTookitSina {
    ...
    private String sp = "";
    private String  location="",errInfo="",uniqueid="",userdomain="";
    /**
     * @author LongJin
    ...
    public boolean preLogin(){
        ...
        try {
            ...
            url += "&su=" + su;
            String content;
            content = HttpTookit.sendGet(url,null,null);
    ...
    public boolean login() {
        if(preLogin()) {
            ...
            map.put("savestate", "7");
            map.put("useticket", "1");
            map.put("pagerefer", "http://login.sina.com.cn/sso/logout.php?entry=miniblog&r=http%3A%2F%2Fweibo.com%2Flogout.php%3Fbackurl%3D%2F");
            map.put("vsnf", "1");
            map.put("su", su);
            ...
            map.put("prelt", "182");
            map.put("url", "http://weibo.com/ajaxlogin.php?framelogin=1&callback=parent.sinaSSOController.feedBackUrlCallBack");
            map.put("domain", "sina.com.cn");
            map.put("returntype", "META");
           // try {
                String content = HttpTookit.sendPost(url,map,null);
                /*System.out.println("content----------" + content);
                ...
                if(m.find()) {
                    ...
                    if(location.contains("reason=")) {//如果你走进了这一步，恭喜报错了
                        ...
                        String result = HttpTookit.sendGet(location,null,null);//.substring(2, location.length()-2)
    ...
    public static void main(String[] args) throws ClientProtocolException, IOException {
        HttpTookitSina weibo = new HttpTookitSina();//账号密码在此就不透露了
        if(weibo.login()) {
            System.out.println("登陆成功！");
            //Document doc = Jsoup.connect("https://m.weibo.cn/api/container/getIndex?containerid=230945_-_Finance_Cardlist_Ranklist&page=1").get();
